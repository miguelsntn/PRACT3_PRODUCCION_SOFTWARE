...
def test_get_by_id_returns_expense():
    ...
    """
    repo = InMemoryExpenseRepository()
    gasto = create_expense(id=1, title="Gasto recuperable", amount=50)
    repo.save(gasto)

    recuperado = repo.get_by_id(1)

    assert recuperado is not None
    assert recuperado.id == 1
    assert recuperado.title == "Gasto recuperable"
    assert recuperado.amount == 50
